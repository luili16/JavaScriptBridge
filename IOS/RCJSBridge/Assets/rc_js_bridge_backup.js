var RCJSBridge = (function() {
	...
	var globalExec = function rcGlobalExec(callback,action,args) {
		...
		rcExecWithErrorCallback(callback,null,"RCActionHandler#12306","globalAction",args);
	}

	var globalExec2 = function rcGlobalExec2(successCallback,errorCallback,action,args) {
		args.push(action);
		//rcExecInternal(callback,"RCActionHandler#12306","globalAction",args);
		rcExecWithErrorCallback(successCallback,errorCallback,"RCActionHandler#12306","globalAction",args);
	}

	var exec = function rcExec(callback, serviceName, action, args) {
		rcExecWithErrorCallback(callback,null,serviceName,action,args);
	}

	var exec2 = function rcExec2(successCallback,errorCallback,serviceName,action,args) {
		console.log("call exec2");
		rcExecWithErrorCallback(successCallback,errorCallback,serviceName,action,args);
	}

	function rcExecWithErrorCallback(successCallback,errorCallback,serviceName,action,args) {
		callbackIndex++;
		...
		if (successCallback !== null) {
			callbackMap[callbackId] = successCallback;
			errorMap[callbackId] = errorCallback;
		} else {
	...
	var nativeCallback = function RCNativeCallback(callbackId, status, keepCallback, argumentsAsJson) {
		...
		var _error = errorMap[callbackId];
